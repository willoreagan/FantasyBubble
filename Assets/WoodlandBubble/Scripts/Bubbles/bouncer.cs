...
//	private OTSpriteBatch spriteBatch = null;  
	...
	void Update () {
		...
		if(transform.position.y <= gameOverBorder && gameObject.layer == 9 ){
	...
	/*	if(Input.GetMouseButtonDown(0) && !isPaused){
			bounceTo(Camera.main.ScreenToWorldPoint(Input.mousePosition));
		}*/

		
/*		if(Vector3.Distance(transform.position, targetPrepare)==0)
			targetPrepare = transform.position;*/
	
		//if(GetComponent<ball>().setTarget)
		//	this.enabled = false;
	}
